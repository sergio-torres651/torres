...
(function ($) {
  ...
  $("#submitButton").on("click", function (event) {
    ...
    if(!senderName.trim() || !senderEmail.trim() || !description.trim()) return;
    $("#submitButton").html('Submitting');
    $("#submitButton").attr('disabled','disabled');
    emailjs.send('service_w9oibvm', 'template_8jtepq3', templateParams).then(
      (response) => {
        console.log('SUCCESS!', response.status, response.text);
        $("#submitButton").html('Submitted successfully');
      },
      (error) => {
        console.log('FAILED...', error);
        $("#submitButton").removeAttr('disabled');
        $("#submitButton").html('Submission failed');
      },
    );
  });
  ...
  $("#submitButton").on("click", function (event) {
    ...
    window.location.href = `mailto:${email}?subject=From ${name}, ${subject}&body=${description}`;
  });
